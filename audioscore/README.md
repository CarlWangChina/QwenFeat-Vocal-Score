...
本说明用于记录如何增加原曲分离清唱数据（高分段）以优化打分模型，供同事备份参考。
...
1. **加载模型**（约第 **33** 行）：在此位置先加载之前部署给松野他们的模型版本权重。
2. **更换路径**（约第 **42**、**43** 行）：将两个路径更换为新生成的训练集和测试集路径。

## 4. 任务背景

目前已由松野处理好数据，阿乐完成标分。任务是将这 2000 首原曲分离出的清唱（85-99 分）作为高分段，与之前的 KTV 唱歌数据（50-85 分）混合，以此优化部署版本的打分效果。
