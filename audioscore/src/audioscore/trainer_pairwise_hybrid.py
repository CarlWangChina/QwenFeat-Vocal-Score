...
from audioscore.dataset import AudioDataset_pairwise_hybrid_feature
...
class Trainer:
    def __init__(self, model, train_json, val_json=None, device=None, local_rank=-1, world_size=-1, data_type="use_audio_feat", save_dir="checkpoints"):
        ...
        self.train_dataset = AudioDataset_pairwise_hybrid_feature(train_json)
        self.val_dataset = AudioDataset_pairwise_hybrid_feature(val_json) if val_json else None
    ...
    def create_dataloaders(self):
        ...
        train_loader = DataLoader(
            ...
            collate_fn=AudioDataset_pairwise_hybrid_feature.collate_fn,
            num_workers=4,
            pin_memory=True
        )
    ...
    def train_step(self, batch, optimizer, epoch):
        ...
        optimizer.zero_grad()
        # print(batch)
        feature_a = batch["feature_a"]
        feature_b = batch["feature_b"]
        # print(feature_a["samoye_pitch"])
        outputs_a = self.model(
            samoye_pitch=feature_a["samoye_pitch"].long().cuda(), 
            samoye_whisper=feature_a["samoye_whisper"].cuda(), 
            samoye_hubert=feature_a["samoye_hubert"].cuda(), 
            mert=feature_a["mert"].cuda(), 
            muq=feature_a["muq"].cuda(), 
            muq_mask = feature_a["mask"]["muq"].cuda()
        )
        outputs_b = self.model(
            samoye_pitch=feature_b["samoye_pitch"].long().cuda(), 
            samoye_whisper=feature_b["samoye_whisper"].cuda(), 
            samoye_hubert=feature_b["samoye_hubert"].cuda(), 
            mert=feature_b["mert"].cuda(), 
            muq=feature_b["muq"].cuda(), 
            muq_mask = feature_b["mask"]["muq"].cuda()
        )
    ...
    def evaluate(self, val_loader):
        ...
        with torch.no_grad():
            ...
            for batch in progress_bar:
                ...
                # 计算损失
                loss = binary_rank_loss(outputs_a, outputs_b, batch["labels"].to(self.device).float())
                # print(outputs_a, outputs_b, batch["labels"], loss)
