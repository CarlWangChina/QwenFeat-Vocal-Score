...
import audioscore.trainer_grl_hubert
...
if __name__ == "__main__":
    ...
    model = audioscore.model.MuqFeatCmpNetwork_GRL_hubert()
    ...
    # 创建训练器
    trainer = audioscore.trainer_grl_hubert.Trainer(
        model=model,
    ...
    trainer.lr = 3e-5
    trainer.epochs = 100
    trainer.save_dir = f"ckpts/MuqFeatCmpNetwork_GRL_hubert/{score_axis}/{args.alpha}/"
    trainer.contact_batch = True
    trainer.use_step_lr = False
    trainer.alpha = args.alpha
    trainer.grl_method = "hubert"
    trainer.gm = args.gm
