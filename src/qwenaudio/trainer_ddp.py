...
def calculate_weights(jsonl_path):
    """统计JSONL文件中各分数类别的权重"""
...
class AudioDataset(Dataset):
    def __init__(self, jsonl_path, processor, max_length=10, prompt="请评价这段歌曲片段的整体听感，用1到5之间的数值进行打分"):
        """
        音频数据集加载器
        Args:
        ...
        self.weights = calculate_weights(jsonl_path)
...
class QwenAudioTrainer:
    def __init__(self, model, processor, train_jsonl, val_jsonl=None, device=None, local_rank=-1, world_size=-1):
        """
        音频评分模型训练器（支持多卡训练）
        Args:
